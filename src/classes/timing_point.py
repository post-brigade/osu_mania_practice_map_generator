...
class TimingPoint:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.beat_length == other.beat_length
            ...
            and self.active_beat_length == other.active_beat_length
        )

    @override
    def __repr__(self):
        if self.uninherited == 1:
                return f"timing point: time:{round(self.time)} meter: {self.meter} beat length: {round(self.active_beat_length)} generate notes: {self.generate_notes} is start/stop: {self.is_start_stop}"
        else:
